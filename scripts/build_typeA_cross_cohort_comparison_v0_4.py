...
METRICS=ROOT/'data/typeA/typeA_common_metrics_v0_3.json'
KHAN=ROOT/'data/typeA/khan_2004_17th_assembly_newleaders_metrics_v1_0.json'
OUT=ROOT/'analysis/typeA_cross_cohort_comparison_v0_4.md'
...
def main():
    ...
    k=json.loads(KHAN.read_text(encoding='utf-8'))
    ...
    order=[
      ...
      ('khan_2004_17th_assembly_newleaders_20','경향신문 2004 국회 뉴리더20'),
      ('donga_2010_2020_100','동아일보 2010 미래100'),
      ('donga_2011_10yr_100','동아일보 2011 미래100')]
    rows=[]
    for key,label in order:
        x=u[key]
        rows.append(f"| {label} | {x['design']} | {x['n']} | {x['baseline_mean']:.2f} | {x['post_peak_mean_assessable']:.2f} | {x['major_n']}/{x['n']} ({pct(x['major_n'],x['n'])}) | {x['apex_n']}/{x['n']} ({pct(x['apex_n'],x['n'])}) | {x['advanced_n']}/{x['n']} ({pct(x['advanced_n'],x['n'])}) |")

    h=u['h21_2004_politics_top10']; kh=u['khan_2004_17th_assembly_newleaders_20']
    text=f"""# Type-A 교차 코호트 비교 v0.4
...
**unique display names:** {q['unique_display_names']} — 동명이인 1쌍(`이미경`)을 별도 canonical person으로 분리
...
| baseline mean | **{h['baseline_mean']:.2f}** | **{kh['baseline_mean']:.2f}** |
| Major | {pct(h['major_n'],h['n'])} | {pct(kh['major_n'],kh['n'])} |
| Apex | {pct(h['apex_n'],h['n'])} | {pct(kh['apex_n'],kh['n'])} |
| Advanced | **{pct(h['advanced_n'],h['n'])}** | **{pct(kh['advanced_n'],kh['n'])}** |
...
- true homonym split: **{q['true_homonym_split_n']}**
- repeated canonical persons: **{q['repeated_person_n']}**
...
- Major: **{naive['major_n']}/245 = {pct(naive['major_n'],245)}**
- Apex: **{naive['apex_n']}/245 = {pct(naive['apex_n'],245)}**
- Advanced: **{naive['advanced_n']}/245 = {pct(naive['advanced_n'],245)}**
