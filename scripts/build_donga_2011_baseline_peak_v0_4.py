...
BASE = TYPEA / 'donga_2011_baseline_peak_through_t0_v0_3.json'
AUDIT = ROOT / 'research/donga_2011_prior_career_audit_v0_3.json'
OUTJ = TYPEA / 'donga_2011_baseline_peak_through_t0_v0_4.json'
OUTC = TYPEA / 'donga_2011_baseline_peak_through_t0_v0_4.csv'
...
def main():
    ...
    assert len(audited_names) == audit['qa']['audited_n'] == 14
    ...
    for p0 in base['people']:
        ...
        if n in audited_names:
            ...
            if rec:
                ...
                corrected.append(n)
                ev = rec
                p['baseline_basis_v0_4'] = 'prior_apex_audit_higher_peak'
            else:
                ev = nochange[n]
                assert ev['baseline_score'] == p['baseline_peak_through_t0'] == 3
                p['baseline_basis_v0_4'] = 'prior_apex_audit_no_score4_identified'
            p['needs_new_person_prior_career_audit'] = False
            p['prior_career_audit_status'] = 'audited_v0_3'
    ...
    assert newly_audited == 14
    assert corrected == [], corrected
    pending = [p['name'] for p in people if p.get('needs_new_person_prior_career_audit')]
    assert len(pending) == 27, len(pending)
    assert all(p['t0_snapshot_scope_score'] == 2 for p in people if p.get('needs_new_person_prior_career_audit'))
    cnt = Counter(p['baseline_peak_through_t0'] for p in people)
    assert cnt == Counter({2: 43, 3: 52, 4: 5}), cnt
    audited_new = 62 - len(pending)
    assert audited_new == 35

    out = {
        'schema_version': 'donga_2011_baseline_peak_through_t0_v0.4',
        'generated': '2026-08-18',
        'status': 'all_new_score3_or4_prior_peak_audits_complete_only_27_score2_new_people_pending',
        'selection_cutoff': '2011-04-01',
        'supersedes': 'data/typeA/donga_2011_baseline_peak_through_t0_v0_3.json',
        't0_ref': base['t0_ref'],
        'audit_refs': [
        ...
        'qa': {
            ...
            'pending_all_t0_score2': all(p['t0_snapshot_scope_score'] == 2 for p in people if p.get('needs_new_person_prior_career_audit')),
    ...
    fields = [
        ...
        'prior_career_audit_confidence','baseline_basis_v0_4'
    ]
