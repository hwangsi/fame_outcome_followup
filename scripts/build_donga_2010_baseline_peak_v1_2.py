...
T0=TYPEA/'donga_2010_t0_snapshot_scope_v1_2.json';OUTJ=TYPEA/'donga_2010_baseline_peak_through_t0_v1_2.json';OUTC=TYPEA/'donga_2010_baseline_peak_through_t0_v1_2.csv'
...
def main():
 ...
 c=Counter(x['baseline_peak_through_t0'] for x in people);assert c==Counter({2:49,3:47,4:3,1:1}),c
 out={'schema_version':'donga_2010_baseline_peak_through_t0_v1.2','generated':'2026-08-18','t0_ref':'data/typeA/donga_2010_t0_snapshot_scope_v1_2.json','supersedes':'data/typeA/donga_2010_baseline_peak_through_t0_v1_1.json','qa':{'total':100,'unique_names':100,'score_counts':{str(k):v for k,v in sorted(c.items())},'mean_baseline_peak':sum(x['baseline_peak_through_t0'] for x in people)/100,'baseline_greater_than_t0_n':sum(x['baseline_minus_t0']>0 for x in people)},'people':people}
