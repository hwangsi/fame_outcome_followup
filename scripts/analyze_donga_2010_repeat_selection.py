...
def main():
    ...
    for score in [1,2,3,4]:
        ...
        if a and b:
            baseline_strata[str(score)]['effects']={
                ...
                'apex_eq4':effect(sum(r['post_t0_peak_score']==4 for r in a),len(a),sum(r['post_t0_peak_score']==4 for r in b),len(b)),
                'advanced':effect(sum(r['advancement_class']=='advanced' for r in a),len(a),sum(r['advancement_class']=='advanced' for r in b),len(b))
            }

    # Transition-specific comparison: baseline=2 advancement means reaching >=3;
    # baseline=3 advancement means reaching apex=4. Baseline=4 has no upward room.
    transition_strata={}
    for score in [2,3]:
        a=[r for r in rr if r['baseline_peak_through_t0']==score]
        b=[r for r in ss if r['baseline_peak_through_t0']==score]
        ah=sum(r['advancement_class']=='advanced' for r in a)
        bh=sum(r['advancement_class']=='advanced' for r in b)
        transition_strata[str(score)]={
            'repeat_n':len(a),'repeat_advanced_n':ah,
            'single_n':len(b),'single_advanced_n':bh,
            'effect':effect(ah,len(a),bh,len(b))
        }
    ...
    out={
        'schema_version':'donga_2010_2011_repeat_selection_metrics_v0.2',
        ...
        'baseline_high_ge3_comparison':{'repeat_n':baseline_high_r,'single_n':baseline_high_s,**baseline_effect},
        ...
        'guardrails':[
            ...
            'Baseline=4 has no upward room on the locked 0-4 scale and is excluded from transition-specific upward comparisons.'
        ]
    }
    OUT_JSON.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')

    lines=['# 동아일보 2010↔2011 반복선정 효과 분석 v0.2','',
           '**분석일:** 2026-08-18  ','**Exposure:** 2010 선정자 중 2011에도 다시 선정되었는가  ','**Repeat:** 38명 / **2010-only:** 62명','',
    ...
    lines += ['', '## 2. Baseline imbalance','',
              f"반복선정군 baseline scope≥3는 **{baseline_high_r}/{len(rr)} ({pct(baseline_high_r/len(rr))})**, 2010-only군은 **{baseline_high_s}/{len(ss)} ({pct(baseline_high_s/len(ss))})**이다.",
              '', f"Baseline-high RR = **{f3(baseline_effect['risk_ratio'])}**, Fisher p = **{f3(baseline_effect['fisher_two_sided_p'])}**.",
              '', '## 3. 같은 baseline 안에서의 실제 상승','',
              '| baseline | repeat advanced | single advanced | RR | Fisher p |','|---:|---:|---:|---:|---:|']
    for score in ['2','3']:
        x=transition_strata[score]; e=x['effect']
        lines.append(f"| {score} | {x['repeat_advanced_n']}/{x['repeat_n']} ({pct(e['repeat_rate'])}) | {x['single_advanced_n']}/{x['single_n']} ({pct(e['single_rate'])}) | {f3(e['risk_ratio'])} | {f3(e['fisher_two_sided_p'])} |")
    lines += ['', 'baseline=2에서는 advancement가 scope≥3 진입을 뜻하고, baseline=3에서는 apex=4 진입을 뜻한다. baseline=4는 척도상 더 올라갈 공간이 없어 이 비교에서 제외한다.',
              '', '## 4. 분야별 exploratory comparison','',
    ...
    lines += ['', '## 5. Interpretation guardrails','',
              ...
              '- unadjusted major/apex 차이는 baseline strength가 크게 섞여 있다.',
              '- 같은 baseline stratum의 advancement 비교가 반복선정의 추가적인 상승 신호에 더 가깝다.',
              '- 작은 표본과 coarse 0-4 scale 때문에 결과는 exploratory로 해석한다.',
              '', '## 6. Reproducibility','',
    ...
    print(json.dumps({
        ...
        'repeat':sr,'single':ssumm,'effects':effects,
        'baseline_high_ge3':{'repeat_n':baseline_high_r,'single_n':baseline_high_s,**baseline_effect},
        'transition_strata':transition_strata,
        'by_baseline_score':baseline_strata,
        'by_editorial_category':category
